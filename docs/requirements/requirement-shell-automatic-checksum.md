...
This requirement is the **project Single Source of Truth** for **automatic companion-digest integrity** of the git-sync POSIX shell tool: downloading a SHA-256 sidecar next to the install channel, verifying install/self-update downloads, and reporting the process **transparently** (companion **link**, expected **value**, and verification **result**).
...
**Must not confuse with:** Embedding a hash of `./git-sync` *inside* `./git-sync`; requiring operators to set `CHECKSUM` for every install; claiming independent host authenticity from same-channel SHA-256 alone.

---

### 1.1 Human-facing

**In one sentence:** Install and self-update download a SHA-256 sidecar next to the script and tell you the **link**, expected **value**, and **result**.

| Box | Meaning | Example |
|-----|---------|---------|
| You / this login | You do **not** need to export `CHECKSUM` for a normal install. | `curl … \| sh` |
| The other role | Publisher ships `git-sync.sha256` beside `./git-sync`. | companion file |
| Not this file | Help/about listing `CHECKSUM` (forbidden). | `git-sync help` |

| Includes | Excludes |
|----------|----------|
| Automatic `${SCRIPT_URL}.sha256` | Embedding a hash of the script inside the script |
| Match → continue; mismatch → abort; missing → warn | Same-origin pin as “higher assurance” |

| Surface | What you open | What for |
|---------|---------------|----------|
| `./git-sync` | ship unit | verify on download |
| `git-sync.sha256` | companion | publisher digest |

| You do… | What it means | What you type |
|---------|---------------|---------------|
| Install | The program fetches the sidecar itself and prints link / value / result in human mode. | `curl -fsSL https://raw.githubusercontent.com/cloudgen/git-sync/main/git-sync \| sh` |
...
| **In-repo publisher SSOT** | Companion file **`git-sync.sha256`** (bare SHA-256 hex of `./git-sync`) **MUST** ship next to the installable script for the release channel. |
| **Regenerate on change** | After every edit to `./git-sync` that is published, regenerate `git-sync.sha256` so automatic mode can match. |
...
| **Outside payload** | Pin is env/operator/CI for that process — **MUST NOT** be embedded inside `./git-sync` as a self-hash of that file. |
...
1. Hash of `./git-sync` stored **inside** `./git-sync` as the verify target.  
...
1. Product root **`README.md` MUST** explain **automatic** checksum as the default: algorithm (SHA-256), companion URL pattern, in-repo `git-sync.sha256`, match / mismatch / missing outcomes.  
...
| **Channel SSOT** | `SCRIPT_URL` default `https://raw.githubusercontent.com/${REPO_USER}/${REPO_NAME}/main/${APP_NAME}` → `https://raw.githubusercontent.com/cloudgen/git-sync/main/git-sync` |
| **Companion URL** | `${SCRIPT_URL}.sha256` → `https://raw.githubusercontent.com/cloudgen/git-sync/main/git-sync.sha256` |
| **In-repo companion** | `git-sync.sha256` (bare 64-char hex) |
...
**This requirement:** Companion verify runs as this login. Do not require `sudo` to fetch or check the sidecar.
...
2. Embed the expected SHA-256 of `./git-sync` **inside** `./git-sync` as verification.  
...
5. Document same-origin `CHECKSUM=$(curl …/git-sync.sha256)` as higher assurance than automatic companion verification.  
...
Integrity work for git-sync is **not done** if any of the following fail:
...
| `./git-sync` | Ship unit implementation |
| `git-sync.sha256` | In-repo companion digest |
