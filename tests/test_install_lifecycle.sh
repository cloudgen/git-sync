...
run_test_install_lifecycle() {
    ...
    mkdir -p "${_global_bin}"
    cp "${SCRIPT}" "${_global_bin}/git-sync"
    chmod +x "${_global_bin}/git-sync"
    _out=$(
    ...
    _older="${CI_CHANNEL_DIR}/git-sync"
    # shellcheck disable=SC2016
    sed "s/^VERSION=\"${PRODUCT_VERSION}\"/VERSION=\"0.9.0\"/" "${SCRIPT}" > "${_older}"
    printf '%s\n' "$(sha256sum "${_older}" | awk '{print $1}')" > "${CI_CHANNEL_DIR}/git-sync.sha256"
