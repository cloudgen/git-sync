...
run_test_cli() {
    ...
    sh -n "${SCRIPT}"
    _syn=$?
    assert_eq "sh -n git-sync (syntax)" 0 "$_syn"

    # --- companion digest matches ship unit ---
    if [ -f "${REPO_ROOT}/git-sync.sha256" ]; then
        # First whitespace-separated field only (never glue HASH+filename)
        _expected=$(awk '{print $1; exit}' "${REPO_ROOT}/git-sync.sha256")
        _actual=$(sha256sum "${SCRIPT}" | awk '{print $1}')
        assert_eq "TP-CLI-01 TP-CSUM-01 git-sync.sha256 matches ./git-sync" "$_expected" "$_actual"
    else
        t_fail "TP-CLI-01 TP-CSUM-01 git-sync.sha256 missing at repo root"
    fi
    ...
    assert_contains "version human mentions app" "$_out" "git-sync"
    ...
    assert_contains "version --json app" "$_out" '"app":"git-sync"'
    ...
    assert_contains "TP-CLI-03 help lists sync (domain)" "$_out" "sync"
    ...
    assert_file_missing "zero-arg failed install left no binary" "${CI_USER_BIN}/git-sync"
    ci_cleanup_env
    ...
    cp "${SCRIPT}" "${CI_USER_BIN}/git-sync"
    chmod +x "${CI_USER_BIN}/git-sync"
    ...
    assert_file_exists "binary remains without --force" "${CI_USER_BIN}/git-sync"
    ci_cleanup_env
}
