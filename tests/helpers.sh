# =============================================================================
# tests/helpers.sh — shared assertions for git-sync CI tests
# =============================================================================
...
# --- isolation helpers ---
# Start a local HTTP channel serving REPO_ROOT/git-sync (+ .sha256).
# Sets: CI_HTTP_PID, CI_SCRIPT_URL, CI_CHANNEL_DIR, CI_PORT
# Caller must call ci_stop_channel on cleanup.
ci_start_channel() {
    CI_CHANNEL_DIR=$(mktemp -d "${TMPDIR:-/tmp}/gs-channel.XXXXXX")
...
ci_isolated_env() {
    CI_HOME=$(mktemp -d "${TMPDIR:-/tmp}/gs-home.XXXXXX")
